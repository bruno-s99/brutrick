...
def focal_loss(preds,gt):
    ...
    for pre in preds:
        ...
        neg_loss=tf.reduce_sum(input_tensor=tf.pow((1-gt),4)*tf.pow(neg_weight,2)*tf.math.log((1-pre)))
        loss=loss-(pos_loss+neg_loss)/(num_pos+tf.convert_to_tensor(value=1e-4))
    return loss
def tag_loss(tag0, tag1, mask):
    ...
    num  = tf.reduce_sum(input_tensor=mask)


    tag_mean = (tag0 + tag1) / 2

    tag0 = tf.pow((tag0 - tag_mean) , 2) / (num + tf.convert_to_tensor(value=1e-4))
    #tag0_mask=tf.where(tf.equal(mask,1),ones,zeros)
    tag0 = tf.reduce_sum(input_tensor=tag0*mask)

    tag1 = tf.pow((tag1 - tag_mean), 2) / (num + tf.convert_to_tensor(value=1e-4))
    #tag1_mask=tf.where(tf.equal(mask,1),ones,zeros)
    tag1 = tf.reduce_sum(input_tensor=tag1*mask)
    pull = tag0 + tag1
    #push
    dist_mask=tf.reshape(mask,(tf.shape(input=mask)[0],1,tf.shape(input=mask)[1]))+tf.reshape(mask,(tf.shape(input=mask)[0],tf.shape(input=mask)[1],1))
    dist_zeros=tf.zeros_like(dist_mask)
    dist_ones=tf.ones_like(dist_mask)
    dist_mask=tf.compat.v1.where(tf.equal(dist_mask,2),dist_ones,dist_zeros)
    num2=num*(num-1)
    dist=tf.reshape(tag_mean,(tf.shape(input=tag_mean)[0],1,tf.shape(input=tag_mean)[1]))-tf.reshape(tag_mean,(tf.shape(input=tag_mean)[0],tf.shape(input=tag_mean)[1],1))
    #dist=-tf.pow(dist,2)
    dist=1-tf.abs(dist)
    dist=tf.nn.relu(dist)
    dist=dist-1 / (num + tf.convert_to_tensor(value=1e-4))
    dist=dist / (num2 + tf.convert_to_tensor(value=1e-4))
    dist=tf.multiply(dist_mask,dist)
    push=tf.reduce_sum(input_tensor=dist)
    return pull, push
...
def offset_loss(offset, gt_offset, mask):
    ...
    offset_loss = offset_loss / (num + tf.convert_to_tensor(value=1e-4))
    offset_loss=tf.reduce_sum(input_tensor=tf.multiply(offset_loss,mask))
    return offset_loss
