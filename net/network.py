...
class NetWork():
    ...
    def decode(self,heat_tl,heat_br,tag_tl,tag_br,offset_tl,offset_br,k=100,ae_threshold=0.5,num_dets=1000):
        ...
        mask_scores=tf.compat.v1.where(tf.cast(tf.equal(class_tl,class_br),tf.int32)>0,scores,invalid)
        mask_scores=tf.compat.v1.where(tf.less(dists,ae_threshold),mask_scores,invalid)
        mask_scores=tf.compat.v1.where(tf.less(x_tl,x_br),mask_scores,invalid)
        mask_scores=tf.compat.v1.where(tf.less(y_tl,y_br),mask_scores,invalid)
