...
import numpy as np
def nms(heat):
    hmax=tf.nn.max_pool2d(input=heat,ksize=[1,3,3,1],strides=[1,1,1,1],padding='SAME')
...
def expand_copy(feature_map,k,inter=False):
    ...
    if inter:
        feature_map=tf.transpose(a=temp,perm=(0,2,1))
    else:
        feature_map=temp
